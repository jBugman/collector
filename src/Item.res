module ItemClass = {
  ...
  let fromTypeLine = (line: string): t => {
    ...
    switch true {
    | _ if contains("Jewel", words) => Jewel
    | _ if contains("Amulet", words) => Jewelry
    | _ if contains("Flask", words) => Flask
    | _ => NULL
    }
  }
  ...
  let isArmour = (line: string): bool => {
    ...
    hasPrefix("Armour:") || (hasPrefix("Evasion Rating:") || hasPrefix("Energy Shield:"))
  }
}
...
let blocks = (text: string): array<block> => {
  let unusablePrefix = %re("/You cannot use this item\\. Its stats will be ignored\\n--------\\n/")
  let separator = "--------"
  text
...
let rarity = (line: string): string => line |> Utils.trimPrefix("Rarity:") |> Js.String.trim
...
let generalizeMod = (line: rawExplicitMod): itemMod => {
  ...
  let idx = ref(0)
  let name = Js.String.unsafeReplaceBy0(
    valueRegex,
    (match_, _, _) => {
      let x = float_of_string(match_)
      let i = idx.contents
...
@send external replaceCustom: (string, Js.Re.t, (string, string, option<string>, string) => string) => string =
  "replace"
...
let generalizeModRange = (line: rawRange): modRange => {
  ...
  let rangeRegex = %re("/([+-]?\\((-?[0-9.]+)[-–](-?[0-9.]+)\\)|([+-]?[0-9.]+))/giu")
  let values = Dict.empty()
  let idx = ref(0)
  ...
  let name = line -> replaceCustom(rangeRegex, (_, _, range1, range2) =>
    switch range1 {
    | Some(range1) =>
...
let compareItemStats = (mods: array<rawExplicitMod>, ranges: array<rawRange>): anyItemScores => {
  module Dict = Js.Dict
  ...
  let modList = Set.intersect(
    mods |> Dict.keys |> Set.fromArray,
    ranges |> Dict.keys |> Set.fromArray,
  )

  let modCount = Set.size(modList)
  let sc = modCount !== Utils.dictSize(mods) || modCount !== Utils.dictSize(ranges)
  sc
    ? Legacy
    : {
        let scores =
          ...
          |> Set.toArray
          |> Array.map(key => {
            let score = scaleMod(Dict.unsafeGet(mods, key), Dict.unsafeGet(ranges, key))
            switch score {
            | Score(x) => Score((key, x))
        ...
        let hasOutOfBoundsScore = Belt.Array.some(scores, x =>
          switch x {
          | OutOfBounds => true
          | _ => false
          }
        ...
        hasOutOfBoundsScore
          ? Legacy
          : {
              let constantsOnly = Belt.Array.every(scores, x =>
                switch x {
                | Constant => true
                | _ => false
                }
              ...
              constantsOnly
                ? Scores({mods: Dict.fromList(list{}), score: 1.0})
                : {
                    let totalScore =
                    ...
                    let usefulScores = Belt.Array.keepMap(scores, x =>
                      switch x {
...
let compareItemStatsNullable = (
  mods: array<rawExplicitMod>,
  ranges: array<rawRange>,
